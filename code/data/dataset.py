...
class TileDataset(Dataset):
    ...
    def __init__(self, df, img_dir, msk_dir, transforms=None):
        """
        Args:
            df (pandas dataframe): file_names.
            img_dir (str): Directory to zipped tiles.
            msk_dir (str): Directory to zipped mask.
            transforms (albumentation transforms, optional) : Optional transform to be applied on a sample. Defaults to None.
        """
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.cvtColor(cv2.imread(os.path.join(self.img_dir, tile_name)),
                           cv2.COLOR_BGR2RGB
                           )
        mask = cv2.imread(os.path.join(self.msk_dir, tile_name),
...
class PredictFromImgDataset(Dataset):
    def __init__(self, original_img_path, mask_name=None,
        ...
        if mask_name is not None:
            df_mask = pd.read_csv('../input/hubmap-kidney-segmentation/train.csv')
    ...
    def enc2mask(self):
        msk = np.zeros(self.orig_size[0]*self.orig_size[1],
                       dtype=np.uint8)
